...
export default function Home() {
    ...
    const fetchDashboardData = async (page: number) => {
        try {
            setLoading(true);
            const response = await fetch(`${API_BASE_URL}/api/dashboard?page=${page}`);
            ...
            if (result.success) {
                const usersWithStreak = result.data
                    .map((user: DashboardUser) => {
                        let currentStreak = 0;
                        const reversedTasks: TaskData[] = [...user.taskData].reverse();

                        for (const task of reversedTasks) {
                            if (task.status === "completed") {
                                currentStreak++;
                            } else if (task.status === "not_joined" || task.status === "future") {
                                continue;
                            } else {
                                break;
                            }
                    ...
                    .sort((a, b) => b.percentage - a.percentage); // ✅ Fixed: now `a` and `b` are inferred correctly
