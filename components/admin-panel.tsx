...
export default function AdminPanel() {
    ...
    if (telegramApp) {
      ...
      const applyTheme = (telegramApp: TelegramWebApp) => {
        if (typeof document !== "undefined") {
          document.body.style.backgroundColor = telegramApp.themeParams.bg_color || "#ffffff"
          document.body.style.color = telegramApp.themeParams.text_color || "#000000"
        }
      }
  ...
  const handleEditTask = async (taskId: number, date: string, description: string) => {
    try {
  ...
  const handleDeleteUser = async (userId: number) => {
    if (!confirm("Haqiqatan ham ushbu foydalanuvchini o'chirishni xohlaysizmi?")) return
  ...
  const handleConfirmPayment = async (userId: number, newPaymentStatus: boolean) => {
    try {
  ...
  const openConfirmPaymentModal = (userId: number, userName: string, newPaymentStatus: boolean) => {
  ...
  return (
    <div className="min-h-screen bg-[var(--tg-theme-bg-color)] text-[var(--tg-theme-text-color)] p-4 transition-all duration-300">
      <div className="max-w-md mx-auto">
        <h1 className="text-2xl font-bold mb-6 text-center">Book Worms Admin Paneli</h1>

        {error && <ErrorMessage message={error} />}

        <AddTaskForm onAddTask={handleAddTask} tg={tg} />

        <TaskList tasks={tasks} onEditTask={openEditModal} onDeleteTask={handleDeleteTask} />

        <UserList users={users} onMarkPaid={openConfirmPaymentModal} onDeleteUser={handleDeleteUser} />
      </div>
      ...
      {confirmingPayment && (
        <ConfirmPaymentModal
          ...
          onConfirm={() => handleConfirmPayment(confirmingPayment.userId, confirmingPayment.newPaymentStatus)}
        />
      )}
    </div>
  )
